async function testAttorneyAppointments() {
    try {
        ...
        const loginResponse = await fetch('https://justicepoint-backend.onrender.com/attorney/login', {
            method: 'POST',
        ...
        if (loginData.token) {
            ...
            const apptResponse = await fetch('https://justicepoint-backend.onrender.com/attorney/appointments', {
                headers: { 
        ...
        const krishnaLogin = await fetch('https://justicepoint-backend.onrender.com/attorney/login', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        if (krishnaData.token) {
            ...
            const krishnaApptResponse = await fetch('https://justicepoint-backend.onrender.com/attorney/appointments', {
                headers: { 
